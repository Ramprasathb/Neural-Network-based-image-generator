...
public class ImageApproximator_Util {
	
	public static double[][][] imageToMatrix(String filename) {
		PApplet p = new PApplet();
		PImage img = p.loadImage(filename);
		double[][][] result = new double[img.height][img.width][3];
		for (int h = 0; h < img.height; h++) {
			for (int w = 0; w < img.width; w++) {
				result[h][w] = intToRGB(img.get(w, h));
			}
		}
		return result;
	}
	
	public static void matrixToImage(double[][][] matrix, String filepath) {
		PApplet p = new PApplet();
	...
	public static PImage matrixToImage(double[][][] matrix) {
		PApplet p = new PApplet();
		PImage img = p.createImage(matrix[0].length,matrix.length, PConstants.RGB);
		for (int y = 0; y < matrix.length; y++) {
			for (int x = 0; x < matrix[0].length; x++) {
	...
	private static double[] intToRGB(int i) {
		i += 16777216;
